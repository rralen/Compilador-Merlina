...
# Modificar la función draw_shapes
def draw_shapes(code):
    canvas.delete("shape")  # Limpiar formas anteriores

    # Expresiones regulares para los rectángulos, círculos, triángulos y pentágonos
...
# Dibujar pentágonos
    ...
    for match in hexagon_matches:
        size = float(match[0])  # Tamaño del hexágono en cm
        pos_x = int(match[2])  # Posición en x
        pos_y = int(match[3])  # Posición en y
        ...
        draw_hexagon(start_x, start_y, size, scale_factor)  # Asegúrate de pasar scale_factor aquí
...
def draw_hexagon(start_x, start_y, size, scale_factor):
    # Convertir tamaño de cm a píxeles y aplicar el factor de escala
    hexagon_size = size * CM_TO_PX * scale_factor  
    points = []

    # Calcular los puntos del hexágono
    for i in range(6):
        angle = pi / 2 + 2 * pi * i / 6  # Rotar 90 grados para alinear
        x = (start_x * scale_factor) + (hexagon_size * cos(angle))  # Escalar start_x
        y = (start_y * scale_factor) + (hexagon_size * sin(angle))  # Escalar start_y
        points.append((x, y))
...
def save_code():
    try:
        code_file = filedialog.asksaveasfilename(defaultextension=".txt", filetypes=[("Text files", "*.txt")])
        if not code_file:
            return
        ...
        messagebox.showinfo("Éxito", "Código guardado exitosamente.")
    except Exception as e:
        messagebox.showerror("Error", f"Ocurrió un error al guardar el código: {str(e)}")
